...
// NewYandex 创建 Yandex 搜索模块
func NewYandex(cfg *config.Config) *Yandex {
	return &Yandex{
		Search:    core.NewSearch("YandexSearch", cfg),
		initURL:   "https://yandex.com/",
...
// Run 执行搜索
func (y *Yandex) Run(domain string) ([]string, error) {
	y.SetDomain(domain)
	y.Begin()
	defer y.Finish()
...
// search 执行搜索
func (y *Yandex) search(domain, filteredSubdomain string) error {
	pageNum := 0 // 二次搜索重新置0
	...
	for {
		...
		params.Set("p", fmt.Sprintf("%d", pageNum))
		params.Set("numdoc", fmt.Sprintf("%d", y.GetPerPageNum()))
